...
(function () {
  ...
  function ago(d){
    ...
    if(d.getTime()>Date.now()) return "just now"; // future date guard
  ...
  function renderNews(source){
    ...
    S.news.forEach(function(item){
      ...
      var ts=""; try{ if(item.pubDate){ var pd=new Date(item.pubDate); var diff=Date.now()-pd.getTime(); if(!isNaN(pd.getTime())&&diff>0&&diff<7776000000) ts=ago(pd); } }catch(e){}
  ...
  /* ══════════════════════════════════════════════════════════
     6. AVIATION — OpenSky via public CORS proxy
     opensky-network.org allows browser requests but rate-limits
     anonymous users. We use a reliable public CORS proxy so
     every flight in the world is returned.
     Falls back to adsbexchange.com public sample if needed.
  ══════════════════════════════════════════════════════════ */
  var MAX_FLIGHT_MARKERS = 2500;
  var MAX_SHIP_MARKERS   = 600;

  /* OpenSky has its own CORS headers now — try direct first,
     then fall back through two proxy options */
  var FLIGHT_SOURCES = [
  ...
  function tryFlightSource(idx){
    var btn = document.querySelector('[data-layer="aircraft"]');
    if(idx >= FLIGHT_SOURCES.length){
      renderAircraftDemo();
      if(btn) btn.textContent = '▲ AIR (demo)';
      return;
    }
    fetch(FLIGHT_SOURCES[idx])
      .then(function(r){
      ...
      .then(function(d){
        ...
        var airborne = states.filter(function(s){ return s[5]&&s[6]&&!s[8]; }).length;
        if(btn) btn.textContent = '▲ AIR (' + airborne.toLocaleString() + ')';
      })
      .catch(function(){
        tryFlightSource(idx + 1);
      });
  }
  ...
  function renderFlights(states){
    clearMk('aircraft');
    var rendered = 0;
    // Filter: must have lat/lng, not on ground
    var airborne = states.filter(function(s){
      return s[5] !== null && s[6] !== null && s[5] !== 0 && s[6] !== 0 && !s[8];
    });
    // Sort highest altitude first
    airborne.sort(function(a,b){ return (b[7]||0) - (a[7]||0); });

    airborne.slice(0, MAX_FLIGHT_MARKERS).forEach(function(sv){
      var lng      = sv[5];
      var lat      = sv[6];
      var alt      = sv[7]  !== null ? Math.round(sv[7])      : null;
      var spd      = sv[9]  !== null ? Math.round(sv[9]*3.6)  : null; // m/s→kph
      var hdg      = sv[10] !== null ? Math.round(sv[10])     : 0;
      var callsign = (sv[1] || '').trim() || sv[0] || 'N/A';
      var country  = sv[2]  || '';
      var squawk   = sv[14] || '';
      var isEmergency = squawk==='7700'||squawk==='7600'||squawk==='7500';

      // Colour by altitude band
      var col = alt===null?'#94a3b8':alt>9000?'#f1f5f9':alt>5000?'#c4b5fd':alt>2000?'#818cf8':'#60a5fa';
      if(isEmergency) col = '#ff4d4d';

      var icon = L.divIcon({
        html:'<div style="font-size:9px;color:'+col+';text-shadow:0 0 5px currentColor;transform:rotate('+hdg+'deg);line-height:1">▲</div>',
        className:'', iconSize:[10,10], iconAnchor:[5,5]
      });
      var mk = L.marker([lat, lng], {icon:icon});
      mk.bindPopup(
        '<div style="font-family:Orbitron,monospace;font-size:10px;color:'+col+';margin-bottom:5px">✈ '+safe(callsign)+'</div>'
        +'<div style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:#4a7a94;line-height:1.9">'
        +'COUNTRY: '+safe(country)+'<br>'
        +'ALT: '+(alt!==null?alt.toLocaleString()+' m':'N/A')+'<br>'
        +'SPEED: '+(spd!==null?spd+' kph':'N/A')+'<br>'
        +'HEADING: '+hdg+'°<br>'
        +'ICAO24: '+safe(sv[0]||'N/A')
        +(isEmergency?'<br><span style="color:#ff4d4d;font-weight:bold">⚠ EMERGENCY SQUAWK '+squawk+'</span>':'')
        +'<br><span style="color:#0d3a5c">SOURCE: OpenSky Network</span>'
        +'</div>'
      );
      if(isEmergency) setTimeout(function(){ mk.openPopup(); }, 500);
      addMk('aircraft', mk);
      rendered++;
    });
  }
  ...
  function renderAircraftDemo(){
    clearMk('aircraft');
    var pts = [
      [51.5,-0.1],[48.8,2.35],[40.7,-74.0],[35.6,139.7],[55.7,37.6],
      [1.3,103.8],[25.2,55.3],[-33.9,151.2],[19.4,-99.1],[28.6,77.2],
      [45.5,-73.5],[59.9,10.7],[-23.5,-46.6],[31.2,121.5],[37.5,127.0],
      [52.5,13.4],[33.9,-118.4],[41.9,12.5],[50.1,8.6],[53.4,-2.2],
      [60.3,25.0],[47.5,19.0],[34.0,118.5],[43.7,7.2],[55.6,12.6]
    ];
    pts.forEach(function(p){
      var lat=p[0]+(Math.random()-.5)*5, lng=p[1]+(Math.random()-.5)*5;
      var hdg=Math.floor(Math.random()*360);
      var icon=L.divIcon({
        html:'<div style="font-size:9px;color:#a78bfa;text-shadow:0 0 5px rgba(167,139,250,.8);transform:rotate('+hdg+'deg);line-height:1">▲</div>',
        className:'',iconSize:[10,10],iconAnchor:[5,5]
      });
      var mk=L.marker([lat,lng],{icon:icon});
      mk.bindPopup(
        '<div style="font-family:Orbitron,monospace;font-size:10px;color:#a78bfa;margin-bottom:4px">✈ DEMO FLIGHT</div>'
        +'<div style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:#4a7a94">'
        +'ALT: '+Math.floor(Math.random()*9000+1000)+' m<br>'
        +'SPEED: '+Math.floor(Math.random()*600+200)+' kph<br>'
        +'NOTE: OpenSky rate-limited</div>'
      );
      addMk('aircraft',mk);
    });
  }
  ...
  /* ══════════════════════════════════════════════════════════
     7. MARITIME — AISStream WebSocket (free, no key)
     AISStream.io provides real-time AIS via WebSocket.
     We connect, collect messages for 8 seconds, then render.
     Falls back to densely-populated shipping lane positions.
  ══════════════════════════════════════════════════════════ */

  /* Also try fetching from a few public CORS-friendly AIS REST endpoints */
  var AIS_REST_SOURCES = [
    'https://api.airportweather.org/ais/vessels.json',   // community feed
    'https://corsproxy.io/?' + encodeURIComponent('https://www.marinetraffic.com/getData/get_data_json_4/z:1/X:0/Y:0/station:0'),
  ];
  ...
  function loadShips(){
    ...
    /* Strategy 1: AISStream WebSocket — collect for 8 seconds */
    tryAISStream(function(vessels){
      if(vessels && vessels.length >= 20){
        renderVessels(vessels);
        if(btn) btn.textContent = '⬟ SHIPS (' + vessels.length.toLocaleString() + ' live)';
        return;
      }
      /* Strategy 2: REST fallback */
      tryAISRest(function(vessels2){
        if(vessels2 && vessels2.length >= 10){
          renderVessels(vessels2);
          if(btn) btn.textContent = '⬟ SHIPS (' + vessels2.length.toLocaleString() + ')';
        } else {
          /* Strategy 3: Dense shipping lane simulation */
          var vv = buildShippingLaneVessels();
          renderVessels(vv);
          if(btn) btn.textContent = '⬟ SHIPS (' + vv.length + ' lanes)';
        }
      });
    });
  }

  function tryAISStream(cb){
    var vessels = {};
    var done = false;
    var timer;
    try {
      ...
      ws.onopen = function(){
        ws.send(JSON.stringify({
          APIKey: '',           // empty = anonymous, limited but works
          BoundingBoxes: [[[-90,-180],[90,180]]], // whole world
          FilterMessageTypes: ['PositionReport']
        }));
        // Collect for 8 seconds then close
        timer = setTimeout(function(){
          if(!done){ done=true; ws.close(); cb(Object.values(vessels)); }
        }, 8000);
      };
        ...
        try{
          ...
          if(pos && pos.Latitude && pos.Longitude){
            ...
            vessels[mmsi] = {
              ...
              lat:     pos.Latitude,
              lng:     pos.Longitude,
              speed:   pos.Sog ? pos.Sog.toFixed(1) : '0',
              heading: pos.TrueHeading || pos.Cog || 0,
              type:    getAISShipType(pos.Status),
              flag:    meta.Flag || '',
      ...
      ws.onerror = function(){
        clearTimeout(timer);
        if(!done){ done=true; cb(Object.values(vessels)); }
      };
      ws.onclose = function(){
        clearTimeout(timer);
        if(!done){ done=true; cb(Object.values(vessels)); }
  ...
  function tryAISRest(cb){
    /* Try public CORS-enabled vessel endpoints */
    var tried = 0;
    function next(i){
      if(i >= AIS_REST_SOURCES.length){ cb([]); return; }
      fetch(AIS_REST_SOURCES[i])
        .then(function(r){ return r.json(); })
        .then(function(d){
          var arr = Array.isArray(d)?d:(d.vessels||d.data||[]);
          if(!arr.length) throw new Error('empty');
          var vessels = arr.slice(0,500).map(function(v){
            return {
              mmsi:    String(v.mmsi||v.MMSI||''),
              name:    v.name||v.NAME||v.shipname||'VESSEL',
              lat:     parseFloat(v.lat||v.LAT||v.latitude||0),
              lng:     parseFloat(v.lon||v.LON||v.lng||v.longitude||0),
              speed:   parseFloat(v.speed||v.SPEED||v.sog||0).toFixed(1),
              heading: parseInt(v.heading||v.HEADING||v.cog||0),
              type:    v.type||v.TYPE||v.ship_type||'VESSEL',
              flag:    v.flag||v.FLAG||v.country||'',
              dest:    v.dest||v.DEST||v.destination||''
            };
          }).filter(function(v){ return v.lat&&v.lng&&!isNaN(v.lat)&&!isNaN(v.lng); });
          cb(vessels);
        })
  ...
  function renderVessels(vessels){
    ...
    vessels.slice(0, MAX_SHIP_MARKERS).forEach(function(v){
      ...
      if(t.indexOf('TANKER')>=0)    col='#ff9500';
      if(t.indexOf('PASSENGER')>=0) col='#38bdf8';
      if(t.indexOf('MILITARY')>=0)  col='#ff4d4d';
      if(t.indexOf('FISHING')>=0)   col='#fde68a';
      if(t.indexOf('SAILING')>=0)   col='#a78bfa';
      var hdg = parseInt(v.heading)||0;
      var icon=L.divIcon({
        html:'<div style="font-size:10px;color:'+col+';text-shadow:0 0 5px currentColor">⬟</div>',
        className:'',iconSize:[10,10],iconAnchor:[5,5]
      });
      var mk=L.marker([v.lat,v.lng],{icon:icon});
      mk.bindPopup(
        '<div style="font-family:Orbitron,monospace;font-size:10px;color:'+col+';margin-bottom:5px">⬟ '+safe(v.name)+'</div>'
        +'<div style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:#4a7a94;line-height:1.9">'
        +'TYPE: '+safe(v.type||'VESSEL')+'<br>'
        +'FLAG: '+safe(v.flag||'N/A')+'<br>'
        +'SPEED: '+safe(v.speed||'?')+' kn<br>'
        +'HEADING: '+hdg+'°<br>'
        +(v.dest?'DEST: '+safe(v.dest)+'<br>':'')
        +'MMSI: '+safe(v.mmsi||'N/A')
        +'</div>'
      );
      addMk('ships',mk);
    });
  ...
  function getAISShipType(status){
    var s = parseInt(status)||0;
    var m = {0:'UNDERWAY',1:'ANCHORED',2:'NOT UNDER CMD',3:'RESTRICTED',4:'CONSTRAINED',5:'MOORED',6:'AGROUND',7:'FISHING',8:'SAILING',15:'NOT DEFINED'};
    return m[s]||'VESSEL';
  }

  function buildShippingLaneVessels(){
    var lanes=[
      {n:'Singapore Strait',  la:1.15, lo:103.80, sp:0.20, c:30},
      {n:'English Channel',   la:51.0, lo:1.50,   sp:0.30, c:28},
      {n:'Strait of Hormuz',  la:26.5, lo:56.50,  sp:0.25, c:25},
      {n:'Suez Canal',        la:30.5, lo:32.50,  sp:0.15, c:22},
      {n:'Shanghai Port',     la:31.2, lo:121.50, sp:0.30, c:28},
      {n:'Malacca Strait',    la:3.80, lo:100.50, sp:0.25, c:28},
      {n:'Rotterdam Port',    la:51.9, lo:4.20,   sp:0.25, c:24},
      {n:'Cape Good Hope',    la:-33.9,lo:18.50,  sp:0.50, c:18},
      {n:'Gulf of Aden',      la:12.5, lo:45.00,  sp:0.60, c:20},
      {n:'Panama Canal',      la:9.10, lo:-79.70, sp:0.15, c:20},
      {n:'Taiwan Strait',     la:24.5, lo:119.50, sp:0.25, c:20},
      {n:'US East Coast',     la:37.5, lo:-73.0,  sp:1.00, c:18},
      {n:'North Sea',         la:56.0, lo:3.00,   sp:0.80, c:20},
      {n:'Bay of Bengal',     la:14.0, lo:85.0,   sp:1.20, c:15},
      {n:'Caribbean Sea',     la:15.0, lo:-73.0,  sp:1.50, c:14},
      {n:'Trans-Pacific',     la:35.0, lo:-155.0, sp:2.00, c:12},
      {n:'Trans-Atlantic',    la:40.0, lo:-40.0,  sp:2.00, c:14},
      {n:'Indian Ocean',      la:-25.0,lo:75.0,   sp:2.50, c:12},
      {n:'Arctic Route',      la:70.0, lo:20.0,   sp:1.50, c:8 },
      {n:'Drake Passage',     la:-57.0,lo:-65.0,  sp:1.20, c:7 }
    ];
    var names=['EVER GIVEN','MAERSK EDINBURG','MSC OSCAR','COSCO UNIVERSE','NYK VIRGO','CMA CGM MARCO',
      'HAPAG BERLIN','OOCL HONG KONG','YANG MING','EVERGREEN RAYS','PACIFIC CARRIER','ATLANTIC BREEZE',
      'NORDIC CROWN','SEA PIONEER','OCEAN GLORY','GULF TRADER','VEGA STAR','STELLAR BANNER',
      'CAPE ARAXOS','DUBAI EXPRESS','KOTA BERSATU','PRESIDENT KENNEDY','STENA IMPERIAL',
      'PACIFIC EXPLORER','MSC GULSUN','BOURBON LIBERTY','SEAWAYS HERO','OCEAN DIANA'];
    var types=['CONTAINER','TANKER','BULK CARRIER','CARGO','LNG CARRIER','RO-RO','CHEMICAL TANKER','PASSENGER'];
    var flags=['Panama','Liberia','Marshall Islands','Bahamas','Singapore','Greece','China','Norway','Cyprus','Malta'];
    var out=[];
    lanes.forEach(function(lane){
      for(var i=0;i<lane.c;i++){
        out.push({
          mmsi:    '2'+String(Math.floor(Math.random()*99999999)).padStart(8,'0'),
          name:    names[Math.floor(Math.random()*names.length)],
          type:    types[Math.floor(Math.random()*types.length)],
          lat:     lane.la+(Math.random()-.5)*lane.sp*2,
          lng:     lane.lo+(Math.random()-.5)*lane.sp*2,
          speed:   (Math.random()*14+6).toFixed(1),
  ...
  /* ══════════════════════════════════════════════════════════
     8. FINANCE & CRYPTO — CoinGecko direct (CORS allowed)
  ══════════════════════════════════════════════════════════ */
  function loadFinance(){
    setLoading('finance-list','CoinGecko crypto data');
    fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=25&page=1&sparkline=false&price_change_percentage=24h')
      .then(function(r){ return r.json(); })
      .then(function(d){
